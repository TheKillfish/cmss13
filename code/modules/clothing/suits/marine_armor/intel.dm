...
/obj/item/clothing/suit/storage/marine/medium/rto/intel/equipped(mob/user, slot, silent) //When XM4 is equipped this removes ACCESSORY_SLOT_FUNCTIONAL as a valid accessory for the uniform
	. = ..()
	if(slot == WEAR_JACKET)
		var/mob/living/carbon/human/human = user
		var/obj/item/clothing/under/uniform = human.w_uniform
		if(uniform?.valid_accessory_slots)
			uniform?.valid_accessory_slots -= ACCESSORY_SLOT_FUNCTIONAL

/obj/item/clothing/suit/storage/marine/medium/rto/intel/unequipped(mob/user, slot) //When unequipped this adds the ACCESSORY_SLOT_FUNCTIONAL back as a valid accessory
	. = ..()
	if(slot == WEAR_JACKET)
		var/mob/living/carbon/human/human = user
		var/obj/item/clothing/under/uniform = human.w_uniform
		if(uniform)
			// Figure out if the uniform originally allowed ACCESSORY_SLOT_FUNCTIONAL
			...
			if(temp_uniform.valid_accessory_slots)
				for(var/allowed in temp_uniform.valid_accessory_slots)
					if(allowed == ACCESSORY_SLOT_FUNCTIONAL)
