...
/mob/living/carbon/xenomorph/verb/hive_status()
	...
	if((!hive.living_hiveleader || SSmapping.configs[GROUND_MAP].map_name == MAP_WHISKEY_OUTPOST) && !hive.allow_reliance_actions) //No Hive status on WO
...
/mob/living/carbon/xenomorph/verb/hive_alliance_status()
	...
	if((!hive.living_hiveleader || Check_WO()) && !hive.allow_reliance_actions) //No Hive status on WO
