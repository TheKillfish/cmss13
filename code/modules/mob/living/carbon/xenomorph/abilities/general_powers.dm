...
/datum/action/xeno_action/activable/info_marker/use_ability(atom/A, mods)
	...
	if(target_turf.z != X.z)
		...
		return
	if(!X.hive.living_hiveleader || X.hive.living_hiveleader.z != X.z)
...
/datum/action/xeno_action/activable/place_construction/use_ability(atom/A)
	...
	var/queen_on_zlevel = !X.hive.living_hiveleader || SSmapping.same_z_map(X.hive.living_hiveleader.z, T.z)
	if(!queen_on_zlevel)
